...
- The discretization stage checks each prohibition and obligation throughout
  the interval between controller updates. It consumes the checked Stage 3
  certificate and tries the linear solver, convex solver, exact symbolic
  checker, and final SMT fallback in that order. Linear proofs record exact
  nonnegative weights, convex proofs record exact dual weights and their
  checked bound, and both certificate forms are replayed independently under
...
| Discretization safety certification | Checks every extracted prohibition and obligation throughout each controller update interval. | Produces a certificate that is rebuilt and independently replayed from the current model and Stage 3 result. |
