#!/usr/bin/env python3
"""Torch reference training for SysML-extracted CPS controllers.

The active artifact pipeline uses the NumPy CPU training program through
``run_pipeline.sh``. This script remains available for comparison experiments:
it runs oracle cloning followed by PPO with the full ``#NeuralRequirement``
shield.
...
def train_oracle(composite, env, iface, device,
    ...
    obs_data, action_data = generate_oracle_data(iface, env, n_samples)
